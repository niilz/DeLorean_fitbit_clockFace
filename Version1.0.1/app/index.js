...
clock.ontick = () => {
  ...
  const day = util.zeroPad(now.getDate(), 2).toString().split('');
  const year = now.getFullYear().toString().split(''); // year needs no Zero-Padding
  const hour = util.zeroPad(now.getHours(), 2).toString().split('');
  const min = util.zeroPad(now.getMinutes(), 2).toString().split('');
 // let sec = util.zeroPad(today.getSeconds()).toString().split('');
...
hr.onreading = () => {
  ...
  heartRate = util.zeroPad(hr.heartRate, 3).toString().split('');
